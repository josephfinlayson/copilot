    ...
    execute: function() {
      ...
      App = React.createClass({
        ...
        render: function() {
          ...
          }, React.createElement("a", {className: "ui-btn ui-btn-icon-right ui-icon-plus"}, "Add New Contact"))), React.createElement(Link, {to: "/sos/addContacts"}, React.createElement("li", {
            "data-icon": "search",
            className: "ui-last-child"
          }, React.createElement("a", {className: "ui-btn ui-btn-icon-right ui-icon-search"}, "Find Contact")))), React.createElement("button", {
              ...
              if (!self.state.map) {
                ...
                (function(global) {
                  ...
                  function onDeviceReady() {
                    $.getScript('https://maps.googleapis.com/maps/api/js?libraries=visualization&callback=onMapsApiLoaded');
                  }
                  global.onMapsApiLoaded = function() {
                    mapProp = {
                      center: new google.maps.LatLng(self.state.lat, self.state.lng),
                      zoom: self.state.mapZoom,
                      mapTypeId: google.maps.MapTypeId.ROADMAP
                    };
                    map = new google.maps.Map(document.getElementById("googleMap"), mapProp);
                    console.log("asdasd");
                    self.setState({map: map});
                    devicePosition = new google.maps.LatLng(self.state.deviceLat, self.state.deviceLng);
                    marker = new google.maps.Marker({position: devicePosition});
                    marker.setMap(map);
                    var list = [];
